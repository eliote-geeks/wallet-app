...
# Usage:
#   HOST=79.137.32.27 USER=ubuntu ./infra/ai-saas/scripts/deploy-vps.sh

HOST="${HOST:-79.137.32.27}"
USER="${USER:-ubuntu}"
IMAGE="ai-saas-portal:dev-local"
APP_URL="${APP_URL:-http://ai-portal-dev.kobo.${HOST}.nip.io}"
...
ssh "${USER}@${HOST}" "rm -rf ${REMOTE_APP_DIR} ${REMOTE_K8S_DIR} && mkdir -p ${REMOTE_APP_DIR} ${REMOTE_K8S_DIR}"
scp -r "${REPO_ROOT}/infra/ai-saas"/* "${USER}@${HOST}:${REMOTE_APP_DIR}/"
scp -r "${REPO_ROOT}/infra/k8s/ai-saas"/* "${USER}@${HOST}:${REMOTE_K8S_DIR}/"
...
ssh "${USER}@${HOST}" "sudo k3s ctr images import /tmp/ai-saas-portal-dev-local.tar"
...
ssh "${USER}@${HOST}" "set -euo pipefail
  ...
  echo WEBHOOK_SECRET=${PAYMENT_WEBHOOK_SECRET} | sudo tee /tmp/ai-saas-secrets.out >/dev/null
"
...
ssh "${USER}@${HOST}" 'sudo kubectl -n ai-saas-dev rollout status statefulset/ai-saas-postgres --timeout=300s && sudo kubectl -n ai-saas-dev rollout status deployment/ai-saas-portal --timeout=300s && sudo kubectl -n ai-saas-dev get pods,svc,ingress && cat /tmp/ai-saas-secrets.out'
